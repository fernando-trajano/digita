/* ==========================================================================
   armazenamento.js — ler e gravar no localStorage.
   ...
   Todas as chaves do digita. começam com "digita:" para não se misturarem
   com as de outros sites que rodem no mesmo endereço.
...
const PREFIXO = 'digita:';

/** As gavetas que o site usa. */
export const CHAVES = {
...
function temLocalStorage() {
  ...
  try {
    ...
    disponivel = false;
    console.warn(
      '[digita] Este navegador não deixa salvar dados. O site funciona, mas ' +
 ...
 * @param {object} padrao  o que devolver quando não há nada salvo
 * @returns {object}
 */
export function ler(chave, padrao = {}) {
  ...
  try {
    ...
    console.warn(`[digita] "${chave}" estava ilegível e foi descartada.`);
    apagar(chave);
    return semNada();
  }
}
...
export function gravar(chave, valor) {
  ...
  try {
    ...
    console.warn(`[digita] Não foi possível salvar "${chave}".`);
    return false;
  }
}
...
/**
 * Tudo o que o site guardou — usado para exportar o progresso em arquivo
 * (passo 17).
 * @returns {object}
 */
